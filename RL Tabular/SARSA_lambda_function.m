function [Q,Pi,AdditionalInfo]=SARSA_lambda_function(MSS,maxEpisodes,TimeWindow,QinitialValue,gamma,Epsilon,Alpha,lambda,ACC_ET,ReductionStep)
...
% lambda = 0.2; %  the decay factor for the eligibility traces
% SAVE_SARSA_LAMBDA=1;  % 1 to save simulation results 0 to not save
...
% [Q,Pi,AdditionalInfo]=SARSA_lambda_function(MSS,maxEpisodes,TimeWindow,gamma,Epsilon,Alpha,lambda,ACC_ET,ReductionStep)
%% OUTPUT
...
ET_start = zeros(MSS.Nstates,MSS.Nactions);
for s=1:MSS.Nstates
...
for i=1:maxEpisodes
    ...
    for t=1:TimeWindow % repeat for a a predefined TimeWindow for non terminating tasks
        ...
        %% Next State Action
        [nsA,~]=Exploit_or_Explore(Q,ns,MSS.IdxCombinationofActionsinStateS,epsilon);
        % update our eligability trace:
        if ACC_ET==1
            ET(cs,csA) = ET(cs,csA)+1; % <- accumulating elagabiity traces ...
        else
            ET(cs,csA) = 1;   % <- replacing elagabiity traces ...
        end
        % Temporal Difference (TD) DT= Rew+ \gamma*Q_{t+1}-Q_{t}
        deltaTime=reward_cs_a + gamma*Q(ns,nsA) - Q(cs,csA);
        % update Q and ET
        Q  = Q + alpha*deltaTime*ET;
        ET = gamma*lambda*ET;
