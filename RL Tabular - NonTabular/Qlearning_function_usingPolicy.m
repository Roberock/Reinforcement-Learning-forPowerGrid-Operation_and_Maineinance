function [Q,Pi,AdditionalInfo]=Qlearning_function_usingPolicy(MSS,maxEpisodes,TimeWindow,QinitialValue,gamma,Alpha,ReductionStep,Policy)
%% EXAMPLE INPUTS 
...
for i=1:maxEpisodes
    ...
    for t=1:TimeWindow % repeat for a a predefined TimeWindow for non terminating tasks
        ...
        CounterUpdating=CounterUpdating+1; % we entered a new updating of Q
          ...
          csA= Policy(cs); % search the maximum among available actions 
          Action=find(MSS.IdxCombinationofActionsinStateS{cs}==csA);
            if isempty(Action); %  if this state has not been explored, take a random action among the available
                Action=randi([1 length(MSS.IdxCombinationofActionsinStateS{cs})]);
                csA=MSS.IdxCombinationofActionsinStateS{cs}(Action);
            end
